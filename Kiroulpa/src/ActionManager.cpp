...
void ActionManager::action(string filname) {
	...
	while(indexAction < actions.size()) {
		...
		switch(typeActionneur) {
			...
			case 'B' : //Envoi en ssh au Gros Robot des points réalisés par Kiroulpa à la balance
				system("python /home/pi/Kiroulpa/EnvoiPointsRealisesBalance.py");
				break;
			case 'C' : //Envoi en ssh au Gros Robot des points réalisés par Kiroulpa à la balance
				system("python /home/pi/Kiroulpa/EnvoiPointsRealisesAccelerateur.py");
